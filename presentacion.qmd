...
## Un poco de quién soy y mi relación con R {.smaller}
...
:::{.notes}
- Datos de inmuebles y carteras de bancos: Estos datos nos los sirven. Son ficheros `.txt` con datos de los distintos inmuebles y sus históricos de precios.
- APIs: De aquí obtenemos datos sociodemográficos como la población, el paro, población en edad de compra o de alquiler, y datos geográficos como mapas.
...
## Datos de inmuebles y carteras de bancos {.smaller}
...
- Al venir de diversas fuentes, hay que unificar ciertas variables, como municipio.
...
- **IGN**: Mapas de provincias, muncipios y distritos en formato `.shp`. Una buena librería para trabajar con este tipo de datos es la librería `sf`.

## Nuestro propio CRM

- Contienen variables de diversos tipos: `character`, `numeric`, `Date`, `logical`.
- Aunque el dato sí esté informado, hay que unificar criterios en variables como municipio o distrito.
...
![](targets_visnetwork_slow_summary.png)
...
```{r}
ggplot(datos[m_y > 2019], aes(x = m_y, y = precio_em2)) + 
  geom_line() +
  zoo::scale_x_yearmon() +
  scale_y_euro +
...
```{r, eval=FALSE}
arima_murcia <- forecast::auto.arima(serie_murcia)
...
Donde A son valores reales y F valores pronosticados. Por ejemplo, si el valor real fuese 1500 € y pronosticado 1578 €, el MAPE en este caso daría `r abs(1500-1578)/1500`, o en porcentaje `r 100*abs(1500-1578)/1500` %.
...
## Tabla de errores y pronósticos {.smaller}
